...
def vigenere_cipher(message, key):
    ...
    for index in range(len(messageIndicesArray)): # range() creates a sequence of numbers representing each ascending index of messageIndicesArray
        if messageIndicesArray[index] != -1 and indexedKeyPatternArray[index] != -1: # Checks both array of message indices and array of key indices to see whether -1 (symbol/space, etc.) exists 
            encryptedMessageIndicesArray.append((messageIndicesArray[index] + indexedKeyPatternArray[index]) % 26)
        else:
            encryptedMessageIndicesArray.append(None)

    vigenere_cipher_output = ""
    for index in range(len(encryptedMessageIndicesArray)):
        if encryptedMessageIndicesArray[index] != None:
            correspondingAlphabetToUse = uppercaseAlphabet if caseInfo[index] == "upper" else lowercaseAlphabet # Outputs the encrypted letter based on whether corresponding caseInfo value is upper or lower 
            # Syntax for ternary operator in Py: (value_if_true) if (condition) else (value_if_false)
            vigenere_cipher_output += correspondingAlphabetToUse[encryptedMessageIndicesArray[index]] 
        else: 
